#include "get_rest.h"
#include <iostream>
#include <sstream>
#include <QDebug>

bool sky_rest::parse_url(const std::string url, std::string& host, short& port, std::string& path)
{
	if(url.empty()) return false;

	std::string tmp, host_port;
	if(url.compare(0, 7, "http://") == 0)
		tmp = url.substr(7);
	else
		tmp = url;
	
	int pos =  tmp.find_first_of('/');
	if(pos > 0){
		host_port = tmp.substr(0, pos);
		path = tmp.substr(pos);
	}
	else{
		host_port = tmp;
		path = "/";
	}
	
	int pos1 = host_port.find_first_of(':');
	if(pos1 > 0){
		host = host_port.substr(0, pos1);
		std::string p = host_port.substr(pos1 + 1, pos - pos1 -1);
		port = atoi(p.c_str());
	}
	else{
		host = host_port;
		port = 80;
	}
	
	return true;
}

//setTransMode设置TCP套接字模式
// 0: bloking mode; !0: nonbloking mode
int sky_rest::setTransMode(int s, int mode)
{
    unsigned long nonbloking_mode = mode;
#if  defined(__WIN32__) || defined(_WIN32)
        //unsigned long nonbloking_mode =1;// 0: bloking mode; !0: nonbloking mode
        if(ioctlsocket(s, FIONBIO, &nonbloking_mode)){
                closeSocket(s);
                return -1;
        }
        //max_s = 0;
#else
        int flags = fcntl(s, F_GETFL, 0);
        if(fcntl(s, F_SETFL, flags | O_NONBLOCK)){
                closeSocket(s);
                return -1;
        }
        //max_s = s+1;
#endif
        return 1;
}
int sky_rest::connect_to_server(const std::string host, const short port, long connect_time_out)
{
	struct hostent* phe;
	struct sockaddr_in sin;
	int s;
	struct   timeval time_out;

	memset(&sin, 0, sizeof(sin));
	sin.sin_family = AF_INET;

	if((phe = gethostbyname(host.c_str())))
		memcpy(&sin.sin_addr, phe->h_addr, phe->h_length);
	else
		sin.sin_addr.s_addr = inet_addr(host.c_str());
	//std::cout << inet_ntoa(sin.sin_addr) << std::endl;
	sin.sin_port = htons(port);
	s = socket(PF_INET, SOCK_STREAM, 0);
	if(s < 0) return -1;
	int max_s;
        if (setTransMode(s, 1) < 0) return -2;  //ioctl error

#if  defined(__WIN32__) || defined(_WIN32)
//	unsigned long nonbloking_mode =1;// 0: bloking mode; !0: nonbloking mode
//	if(ioctlsocket(s, FIONBIO, &nonbloking_mode)){
//		closeSocket(s);
//		return -1;
//	}
	max_s = 0;
#else
//	int flags = fcntl(s, F_GETFL, 0);
//	if(fcntl(s, F_SETFL, flags | O_NONBLOCK)){
//		closeSocket(s);
//		return -1;
//	}
	max_s = s+1;
#endif
        int ret;
        ret = connect(s , (struct sockaddr*) &sin, sizeof(sin));
        if( ret == -1 && REST_EWOULDBLOCK != rest_getLastError()){
		closeSocket(s);
                return -3;
	}
	time_out.tv_sec = connect_time_out;
	time_out.tv_usec = 0;

        fd_set r,w;
        if (ret == 0 )
        {
            if (setTransMode(s,0) < 0) return -2;
            OutputDebugStringA("++++connect_to_server 成功。\r\n");
            return s;
        }

    /*
    while(1)
    {
        FD_ZERO(&r);
        FD_SET(s, &r);
        int ret = select(max_s, 0, &r, 0, &time_out);
        if( ret <= 0 )
        {
            closeSocket(s);
            return -1;
        }
        if(FD_ISSET(s, &r)) break;
    }
    */

    FD_ZERO(&r);
    FD_ZERO(&w);
    FD_SET(s, &r);
    FD_SET(s, &w);
    ret = select(max_s, &r, &w, 0, &time_out);
    if (ret <= 0)
    {
            closesocket(s);
            return -4;  //error
    }
    if (ret == 0)
    {
        closesocket(s);
        return -5;  //timeout
    }

    if (FD_ISSET(s,&w) && FD_ISSET(s,&r))
    {
        if (setTransMode(s, 0) < 0) return -2;
        char buf[2];
        ret = recv(s,buf,0,0);
        if (ret < 0)
        {
            closesocket(s);
            return -6;
        }
        if (ret == 0 && WSAGetLastError() == 0)
        {
            if (setTransMode(s, 0) < 0) return -2;
            OutputDebugStringA("++++connect_to_server 成功。2\r\n");
            return s;
        }
    }
    else if (FD_ISSET(s,&w))
    {
        if (setTransMode(s, 0) < 0) return -2;
//        OutputDebugStringA("++++connect_to_server 成功。3\r\n");

/*
#if  defined(__WIN32__) || defined(_WIN32)
        struct timeval tv;
        tv.tv_sec = 2;
        tv.tv_usec = 0;
        if (setsockopt(s, SOL_SOCKET, SO_RCVTIMEO, (const char *)&tv, sizeof(tv)))
        {
            OutputDebugStringA("++++connect_to_server set timeout error!\r\n");
        }
#endif
*/
        return s;
    }

    return -6;
}

int sky_rest::send_request(int s, const std::string path, const std::string token, const std::string host, const short port)
{
	std::stringstream request;
	if(token.empty())
		request << "GET " << path << " HTTP/1.1\r\n"
				<< "Host: " << host << ":" << port << "\r\n"
                                //<< "Connection: keep-alive\r\n"
                                   << "Connection: close" <<  "\r\n"
				<< "\r\n";
	else
		request << "GET " << path << " HTTP/1.1\r\n"
				<< "Host: " << host << ":" << port << "\r\n"
				<< "Accept:text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8\r\n"
				<< "Cookie: csst_token=" << token << "\r\n"
                                //<< "Connection: keep-alive\r\n"
                                   << "Connection: close" <<  "\r\n"
				<< "\r\n";
	std::string s_req = request.str();
	int ret = send(s, s_req.c_str(), s_req.length(), 0);
        if (ret <= 0)
        {
#ifdef _DEBUG
            char szDebug[2048] = {0};
            sprintf(szDebug, "send_request Error: %d 内容:%s\r\n", WSAGetLastError(),s_req.c_str());
            OutputDebugStringA(szDebug);
#endif
            return ret;
        }
#ifdef _DEBUG
        else
        {
            char szDebug[2048] = {0};
            sprintf(szDebug, "send_request Successed! 内容:%s\r\n", s_req.c_str());
            OutputDebugStringA(szDebug);
        }
 #endif
        //shutdown(s, REST_SEND);
	return ret;
}

int sky_rest::send_post_request(int s, const std::string path, const std::string data, const std::string token, const std::string host, const short port)
{
	std::stringstream request;
	if(token.empty())
		request << "POST " << path << " HTTP/1.1\r\n"
				<< "Host: " << host << ":" << port << "\r\n"
				<< "Content-Type: application/x-www-form-urlencoded\r\n"
				<< "Content-Length: " << data.length() << "\r\n"
				//<< "Connection: keep-alive\r\n"
                                << "Connection: close" <<  "\r\n"
				<< "\r\n"
				<< data;
	else
		request << "POST " << path << " HTTP/1.1\r\n"
				<< "Host: " << host << ":" << port << "\r\n"
				<< "Accept:text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8\r\n"
				<< "Cookie: csst_token=" << token << "\r\n"
				<< "Content-Type: application/x-www-form-urlencoded\r\n"
				<< "Content-Length: " << data.length() << "\r\n"
				//<< "Connection: keep-alive\r\n"
                                << "Connection: close" <<  "\r\n"
				<< "\r\n"
				<< data;
	int ret = send(s, request.str().c_str(), request.str().length(), 0);
        if (ret <= 0)
        {
#ifdef _DEBUG
			char szDebug[2048] = {0};
			sprintf(szDebug, "send_post_request Error: %d 内容:%s\r\n", WSAGetLastError(), request.str().c_str());
			OutputDebugStringA(szDebug);
#endif
		}
#ifdef _DEBUG
        else
        {
            char szDebug[2048] = {0};
            sprintf(szDebug, "send_post_request succesed. 内容:%s\r\n", request.str().c_str());
            OutputDebugStringA(szDebug);
        }
#endif
        //shutdown(s, REST_SEND);
	return ret;
}

int sky_rest::get_response(int s, std::string& result)
{
        std::stringstream ss(std::stringstream::app | std::stringstream::out | std::stringstream::binary);

        char buff[10*1024];
	int received = 0, ret;
	memset(buff, 0, 10*1024);
	while((ret = recv(s, buff, 10*1024, 0)) > 0){
		ss.write(buff,ret);
		received += ret;
/*                if (strstr(buff,"\r\n\r\n"))
                {
#ifdef _DEBUG
                    char szDebug[2048] = {0};
                    sprintf(szDebug,"get_response=%d %s\r\n",received, ss.str().c_str());
                    OutputDebugStringA(szDebug);
#endif
                    break;
                }*/
//		memset(buff, 0, 10*1024);
	}
	if(received > 0) result = ss.str();
	return received;
}

int sky_rest::get_response_body(const std::string response, std::string& body)
{
	int state = -1;
	int pos_header, pos_state;
	if(((pos_state = response.find("\r\n", 0, 2)) > 0)){
		std::string ss = response.substr(8, pos_state - 8);
		const char* s = ss.c_str();
		while(*s == ' ' || *s == '\t') s++;
		std::string n = s;
		pos_state = n.find(' ');
		if(pos_state < 0) pos_state = n.find('\t');
		if(pos_state > 0){
			std::string s_state = n.substr(0, pos_state);
			state = atoi(s_state.c_str());
		}
	}
	if((pos_header = response.find("\r\n\r\n", 0, 4)) > 0){
		body = response.substr(pos_header + 4);
	}
	return state;
}

int sky_rest::get_rest(const std::string url, std::string& body, long connect_time_out, const std::string token)
{
	int result = -1;
	int state = result;
	std::string host, path, response;
	short port;
	int s;
	body = "";
	//qDebug()<< QString::fromStdString(url);
	//qDebug()<< "+++ 1";
	if(parse_url(url, host, port, path)){
		//qDebug()<< "+++ 2";
		s = connect_to_server(host, port, connect_time_out);
		//qDebug()<< "+++ 3";
		if(s > 0){
                    if(send_request(s, path, token, host, port) > 0){
						//qDebug()<< "+++ 4";
                        if(get_response(s, response) > 0){
							//qDebug()<< "+++ 5";
                            state = get_response_body(response, body);
							//qDebug()<< "+++ 6";
                            if(state >= 200 && state < 300)
                                result = 0;
                            else
                                result = -5;
                } else {
                    result = -4;
                }
            } else {
                result = -3;
            }
			closeSocket(s);
        } else {
            result = -2;
        }
	}
	//qDebug()<< "+++ 7";
	return result;
}

int sky_rest::post_rest(const std::string url, const std::string data, std::string& body, long connect_time_out, const std::string token)
{
	int result = -1;
	int state = result;
	std::string host, path, response;
	short port;
	int s;
	body = "";
	//qDebug()<< QString::fromStdString(url);
	//qDebug()<< "+++ 1";
	if(parse_url(url, host, port, path)){
		//qDebug()<< "+++ 2";
		s = connect_to_server(host, port, connect_time_out);
		//qDebug()<< "+++ 3";
		if(s > 0)
		{
			if(send_post_request(s, path, data, token, host, port) > 0)
			{
				//qDebug()<< "+++ 4";
				if(get_response(s, response) > 0)
				{
					//qDebug()<< "+++ 5";
					state = get_response_body(response, body);
					//qDebug()<< "+++ 6";
					if(state >= 200 && state < 300)
						result = 0;
					else
						result = -5;
				}
				else 
				{
					result = -4;
				}
			}
			else
			{
				return -3;
			}
			closeSocket(s);
		}
		else
		{
			return -2;
		}
	}
	//qDebug()<< "+++ 7";
	return result;
}
